export default function validerLangage(langage) {
    const langagesDeProgrammation = [
        "A# .NET",
        "A#",
        "A+",
        "ABAP",
...
"CMS EXEC (EXEC2, EXEC2 40, EXEC2 50)",
"EXEC",
"EXEC 2",
"EXEC 2",
"Extempore",
...
"ZX81 BASIC"
  ...
  for (let i = 0; i < langagesDeProgrammation.length; i++) {
    // Convertir le nom de langage en minuscules
    const nomLangage = langagesDeProgrammation[i].toLowerCase();

    // Si le nom de langage contient la chaîne recherchée,
    // retourner true
    if (nomLangage.indexOf(langage) !== -1) {
      return true;
    }
  }
